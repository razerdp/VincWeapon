//铝热剂
class KFWeap_Throw_Thermite extends KFWeap_ThrownBase;
...
var array<KFProj_Throw_Thermite> DeployedCharges;
...
{
	local Projectile P;
	local KFProj_Throw_Thermite Charge;

	P = super.ProjectileFire();

	Charge = KFProj_Throw_Thermite( P );
	if( Charge != none )
	{
		if(SkinItemId > 0)
		{
...
function SetOriginalValuesFromPickup( KFWeapon PickedUpWeapon )
{
	...
	DeployedCharges = KFWeap_Throw_Thermite(PickedUpWeapon).DeployedCharges;
	NumDeployedCharges = DeployedCharges.Length;
	bForceNetUpdate = true;
